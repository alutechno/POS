	...
	class Mst_pos_member_package extends CI_Controller {
		...
		public function read($id) {
			...
			if ($row) {
				$data = array('id' => $row->id, 'code' => $row->code, 'name' => $row->name,
					...
					'status' => $row->status, 'percent_food' => $row->percent_food,
					'percent_beverage' => $row->percent_beverage,
					'percent_others' => $row->percent_others, 'created_date' => $row->created_date,
		...
		public function create() {
			$data = array('button' => 'Create',
				'action' => site_url('mst_pos_member_package/create_action'),
				'id' => set_value('id'), 'code' => set_value('code'), 'name' => set_value('name'),
				'short_name' => set_value('short_name'), 'description' => set_value('description'),
				'status' => set_value('status'), 'percent_food' => set_value('percent_food'),
				'percent_beverage' => set_value('percent_beverage'),
		...
		public function create_action() {
			...
			if ($this->form_validation->run() == false) {
				...
				$data = array('code' => $this->input->post('code', true),
					...
					'percent_food' => $this->input->post('percent_food', true),
					'percent_beverage' => $this->input->post('percent_beverage', true),
					'percent_others' => $this->input->post('percent_others', true),
		...
		public function update($id) {
			...
			if ($row) {
				$data = array('button' => 'Update',
					'action' => site_url('mst_pos_member_package/update_action'),
					'id' => set_value('id', $row->id), 'code' => set_value('code', $row->code),
					'name' => set_value('name', $row->name),
					'short_name' => set_value('short_name', $row->short_name),
					'description' => set_value('description', $row->description),
					'status' => set_value('status', $row->status),
					'percent_food' => set_value('percent_food', $row->percent_food),
					'percent_beverage' => set_value('percent_beverage', $row->percent_beverage),
					'percent_others' => set_value('percent_others', $row->percent_others),
		...
		public function _rules() {
			...
			$this->form_validation->set_rules('percent_food', 'percent food',
											  'trim|required|numeric');
			$this->form_validation->set_rules('percent_beverage', 'percent beverage',
											  'trim|required|numeric');
			$this->form_validation->set_rules('percent_others', 'percent others',
											  'trim|required|numeric');
