<?php
	...
	class Mst_house_use extends CI_Controller {
		...
		public function read($id) {
			...
			if ($row) {
				$data = array('id' => $row->id, 'code' => $row->code, 'name' => $row->name,
					...
					'status' => $row->status, 'pos_cost_center_id' => $row->pos_cost_center_id,
		...
		public function create() {
			$data = array('button' => 'Create', 'title' => 'House',
				'action' => base_url('master/mst_house_use/create_action'), 'id' => set_value('id'),
		...
		public function update($id) {
			$row = $this->Mst_house_use_model->get_by_id($id);
			if ($row) {
				$data = array('button' => 'Update', 'title' => 'Hause Use',
