<?php
	...
	class Mst_cuisine_recipe extends CI_Controller {
		...
		public function read($id) {
			...
			if ($row) {
				$data = array('id' => $row->id, 'code' => $row->code, 'name' => $row->name,
					...
					'picture_link' => $row->picture_link, 'status' => $row->status,
					'cost_center_id' => $row->cost_center_id, 'meal_time_id' => $row->meal_time_id,
					'reg_style_id' => $row->reg_style_id, 'category_id' => $row->category_id,
					'producing_qty' => $row->producing_qty, 'unit_type_id' => $row->unit_type_id,
					'level' => $row->level, 'product_cost' => $row->product_cost,
		...
		public function create() {
			$data = array('button' => 'Create', 'title' => 'Cuisine Recipe',
				...
				'picture_link' => set_value('picture_link'), 'status' => set_value('status'),
				...
				'unit_type_id' => set_value('unit_type_id'), 'level' => set_value('level'),
		...
		public function create_action() {
			...
			if ($this->form_validation->run() == false) {
				...
				$data = array('code' => $this->input->post('code', true),
					...
					'onhand_suggestion_price' => $this->input->post('onhand_suggestion_price',
																	true),
					'manual_unit_cost' => $this->input->post('manual_unit_cost', true),
					'manual_cost_percent' => $this->input->post('manual_cost_percent', true),
					'manual_suggestion_price' => $this->input->post('manual_suggestion_price',
																	true),);
		...
		public function update($id) {
			$row = $this->Mst_cuisine_recipe_model->get_by_id($id);
			if ($row) {
				$data = array('button' => 'Update', 'title' => 'Cuisine Recipe',
					...
					'makeup_cost_percent' => set_value('makeup_cost_percent',
													   $row->makeup_cost_percent),
					'expected_cost_percent' => set_value('expected_cost_percent',
														 $row->expected_cost_percent),
		...
		public function _rules() {
			...
			$this->form_validation->set_rules('cooking_direction', 'cooking direction',
											  'trim|required');
