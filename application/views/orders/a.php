	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body" style="padding-bottom: 0px;">
				<div class="row">
					<div class="col-lg-12">
						<label id="splitMode"></label>
						<label style="margin: 0 10px;">/</label>
						<label id="splitModeSub"></label>
					</div>
				</div>
				<div class="row">
					<div class="col-lg-12">
						<div class="form-group row" style="margin-bottom:0px;">
							<h5 for="inputKey" class="col-md-2 control-label">
								<label>Split with</label>
							</h5>
							<div class="col-md-4">
								<select class="form-control" id="splitBy">
							...
							<div class="col-md-2 pull-right">
								<input class="form-control" id="splitManualCnt" type="number" min="2" max="10" value="2"/>
							</div>
						</div>
					</div>
				</div>
				<!--				<div class="row" id="splitManual" style="max-height: 200px;overflow-y: auto;">-->
				<!--					?-->
				<!--				</div>-->
				<script>
					var splitManual = $('#splitManual');
					var splitManualCnt = $('#splitManualCnt');
					var splitBy = $('#splitBy');
					var buildSplitManual = function (i) {
						return `
							<div id="split-${i}" class="col-lg-12">
								<div class="form-group row">
									<h5 for="inputKey" class="col-md-1 control-label">
										<label>#</label>
										<label>${i}</label>
									</h5>
									<div class="col-md-5">
									...
									<div class="col-md-2">
										<button class="btn btn-default" style="width: 100%">OK</button>
									</div>
								</div>
							</div>
					    `;
					...
					splitManual.hide();
					splitManualCnt.parent().hide();
					splitManualCnt.on('blur', function () {
						var c = $(this).val();
						if (c > 0 && c < 11) {
							var i = 0;
							splitManual.html('');
							while (i < c) {
								splitManual.append(buildSplitManual(i+1))
								i++;
							}
						}
					});
					splitBy.on('change', function () {
						splitManualCnt.parent().hide();
						if ($(this).val() == 'manual') {
							var c = parseFloat(splitManualCnt.val());
							splitManualCnt.parent().show();
							splitManual.show();
							splitManual.html('');
							if (c > 0) {
								var i = 0;
